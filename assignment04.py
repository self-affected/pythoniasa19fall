...
def task3(text):
    ...
    """
    # todo: write your code here
    return {'digits': len(set(digit for digit in text if digit.isdigit())), 'letters': len(set(letter for letter in text if letter.isalpha()))}
...
def task7(text, words):
    ...
    # todo: write your code here
    import re
    pattern = '.*'.join(sorted(''.join(words)))
    dictionary = ''.join(sorted(text.lower()))
    return bool(re.search(pattern, dictionary))
